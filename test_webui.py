...
def test_update_threshold_values_boundaries():
    ...
    num_slices = 5
    threshold_values, _ = update_threshold_values(input_thresholds, num_slices, filename)

    # Assert that the state is updated
    print(threshold_values)
    print(state.imgThresholds)
    assert state.imgThresholds == [0, 1, 20, 30, 40, 254, 255]


def test_update_threshold_values_limit():
    # Create a dummy state
    state = AppState()
    state.imgThresholds = [0, 10, 20, 30, 40, 255]
    ...
    print(threshold_values)
    print(state.imgThresholds)
    assert state.imgThresholds == [0, 255, 256, 257, 258, 254, 255]
