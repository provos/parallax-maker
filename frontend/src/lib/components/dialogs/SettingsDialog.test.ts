...
import ConfigurationTab from './SettingsDialog.svelte';
...
describe('ConfigurationTab', () => {
  ...
  describe('nav', () => {
    it('starts on the Inpainting section, marked current', () => {
      render(ConfigurationTab);
      expect(screen.getByTestId('settings-nav-inpainting')).toHaveAttribute('aria-current', 'true');
      expect(screen.getByTestId('settings-nav-depth')).not.toHaveAttribute('aria-current');
      expect(uiStore.settingsSection).toBe('inpainting');
    });

    it('clicking a nav link sets uiStore.settingsSection and moves aria-current', async () => {
      render(ConfigurationTab);

      await fireEvent.click(screen.getByTestId('settings-nav-appearance'));

      expect(uiStore.settingsSection).toBe('appearance');
      expect(screen.getByTestId('settings-nav-appearance')).toHaveAttribute('aria-current', 'true');
      expect(screen.getByTestId('settings-nav-inpainting')).not.toHaveAttribute('aria-current');
    });

    it('opening at a section (openSettings) is reflected in the nav', () => {
      uiStore.openSettings('slicing');
      render(ConfigurationTab);
      expect(screen.getByTestId('settings-nav-slicing')).toHaveAttribute('aria-current', 'true');
    });
  });

  describe('depth model', () => {
    it('reads uiStore.depthModel', () => {
      render(ConfigurationTab);
      expect(screen.getByTestId('settings-depth-model')).toHaveValue('dinov2');
    });

    it('without a project, only updates uiStore.depthModel', async () => {
      const fetchMock = vi.fn();
      vi.stubGlobal('fetch', fetchMock);
      render(ConfigurationTab);

      await fireEvent.change(screen.getByTestId('settings-depth-model'), { target: { value: 'midas' } });

      expect(uiStore.depthModel).toBe('midas');
      expect(fetchMock).not.toHaveBeenCalled();
    });

    it('with a project, also PUTs the new depth model to /settings', async () => {
      const projectId = 'appstate-test';
      projectStore.applyView(makeView());
      const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        const url = String(input);
        ...
        if (url === `/api/v1/projects/${projectId}/settings` && method === 'PUT') {
          expect(JSON.parse(init!.body as string)).toEqual({ depthModel: 'midas' });
          return jsonResponse(200, { ...makeView({ revision: 2 }), changed: true });
        }
        if (url.startsWith(`/api/v1/projects/${projectId}/logs`)) return jsonResponse(200, { entries: [], next: 0 });
        throw new Error(`Unexpected fetch: ${method} ${url}`);
      });
      vi.stubGlobal('fetch', fetchMock);
  ...
  describe('num slices', () => {
    it('reads uiStore.pendingNumSlices', () => {
      render(ConfigurationTab);
      expect(screen.getByTestId('settings-num-slices')).toHaveValue('3');
    });

    it('without a project, only updates uiStore.pendingNumSlices', async () => {
      const fetchMock = vi.fn();
      vi.stubGlobal('fetch', fetchMock);
      render(ConfigurationTab);

      await fireEvent.change(screen.getByTestId('settings-num-slices'), { target: { value: '6' } });

      expect(uiStore.pendingNumSlices).toBe(6);
      expect(fetchMock).not.toHaveBeenCalled();
    });

    it('with a project, also PUTs the new count to /slice-count', async () => {
      const projectId = 'appstate-test';
      projectStore.applyView(makeView());
      const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        const url = String(input);
        const method = init?.method ?? 'GET';
        if (url === `/api/v1/projects/${projectId}/slice-count` && method === 'PUT') {
          expect(JSON.parse(init!.body as string)).toEqual({ numSlices: 6 });
          return jsonResponse(200, { ...makeView({ revision: 2 }), changed: true });
        }
        if (url.startsWith(`/api/v1/projects/${projectId}/logs`)) return jsonResponse(200, { entries: [], next: 0 });
        throw new Error(`Unexpected fetch: ${method} ${url}`);
      ...
      await waitFor(() =>
        expect(fetchMock).toHaveBeenCalledWith(
          `/api/v1/projects/${projectId}/slice-count`,
          expect.objectContaining({ method: 'PUT' }),
        ),
      );
    });
  ...
  describe('theme radios', () => {
    it('reflects uiStore.theme with aria-checked', () => {
      render(ConfigurationTab);
    ...
    it('clicking the already-active theme is a no-op (no toggle, no fetch)', async () => {
      const fetchMock = vi.fn();
      vi.stubGlobal('fetch', fetchMock);
      render(ConfigurationTab);

      await fireEvent.click(screen.getByTestId('settings-theme-dark'));

      expect(uiStore.theme).toBe('dark');
      expect(fetchMock).not.toHaveBeenCalled();
    });

    it('clicking the other theme flips uiStore.theme, and persists it with a project', async () => {
      const projectId = 'appstate-test';
      projectStore.applyView(makeView());
      const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        const url = String(input);
        const method = init?.method ?? 'GET';
        if (url === `/api/v1/projects/${projectId}/settings` && method === 'PUT') {
          expect(JSON.parse(init!.body as string)).toEqual({ darkMode: false });
          return jsonResponse(200, { ...makeView({ revision: 2 }), changed: true });
        }
        if (url.startsWith(`/api/v1/projects/${projectId}/logs`)) return jsonResponse(200, { entries: [], next: 0 });
        throw new Error(`Unexpected fetch: ${method} ${url}`);
      ...
      render(ConfigurationTab);
      await fireEvent.click(screen.getByTestId('settings-theme-light'));
  ...
  describe('restore state', () => {
    it('closes the dialog once a different project has loaded', async () => {
      projectStore.applyView(makeView());
      uiStore.openDialog('settings');
      const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        const url = String(input);
        const method = init?.method ?? 'GET';
        if (url === '/api/v1/projects/restore' && method === 'POST') {
          return jsonResponse(200, makeView({ id: 'appstate-restored', revision: 1 }));
        }
        if (url.startsWith('/api/v1/projects/appstate-restored/logs')) return jsonResponse(200, { entries: [], next: 0 });
        throw new Error(`Unexpected fetch: ${method} ${url}`);
      });
      vi.stubGlobal('fetch', fetchMock);

      render(ConfigurationTab);
      const file = new File(['{}'], 'appstate.json', { type: 'application/json' });
      await fireEvent.change(screen.getByTestId('restore-state-input'), { target: { files: [file] } });

      await waitFor(() => expect(uiStore.dialog).toBeNull());
      expect(projectStore.view?.id).toBe('appstate-restored');
    });

    it('leaves the dialog open if the restore fails', async () => {
      projectStore.applyView(makeView());
      uiStore.openDialog('settings');
      const fetchMock = vi.fn(async (input: RequestInfo | URL): Promise<Response> => {
        const url = String(input);
        if (url === '/api/v1/projects/restore') {
          return jsonResponse(400, { error: { code: 'bad_request', message: 'not a valid state file' } });
        }
