...
| `PUT /api/v1/projects/{id}/settings` | `{depthModel?, camera?: {distance, focalLength, maxDistance, pitch?, groundNear?}, meshDisplacement?, darkMode?}` | `200 ProjectView & {changed}` (sync); `update_settings` - unifies `remember_depth_model`/the persist `remember_camera_parameters`/`toggle_dark_mode` into one command; each field is only applied (and the project only re-saved, JSON-only) when both provided and different from the project's current value, exactly matching every one of those callbacks' own unchanged-value guard individually. | `camera.pitch` (degrees, ±60, positive looks up; optional, omitted = unchanged) tilts the reference camera about its x axis: cards stay vertical planes, built by back-projecting the image corners, so the reference view still reproduces the image; `Camera.pitch_for_horizon` converts a horizon row to a pitch.
